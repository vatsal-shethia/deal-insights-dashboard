...
function Dashboard() {
  ...
  const handleAskQuestion = async () => {
    ...
    try {
      const data = await apiFetch(`/api/deals/${dealId}/ask`, {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    const fetchDealData = async () => {
      try {
        // Use deal data passed via navigation state if available
        ...
        if (dealId) {
          const data = await apiFetch(`/api/deals/${dealId}`);
          setDealData(data);
        }
      } catch (error) {
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#faf8f6" }}>
      ...
      <main
        ref={mainRef}
        style={{ maxWidth: "1400px", margin: "0 auto", padding: "2.5rem 3rem" }}
      >
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "1.5rem" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              >
                ...
                <span>
                  📅 {new Date(dealData.dateUploaded).toLocaleDateString()}
                </span>
          ...
          >
            ...
            >
              <div>
                ...
                >
                  {dealData.dealSummary.healthScore}/100{" "}
                  <span style={{ fontSize: "1.25rem" }}>
                    {dealData.dealSummary.healthStatus}
                  </span>
                </p>
              </div>
              ...
              <div>
                ...
                <p
                  style={{
                    ...
                    fontWeight: "300",
                    color:
                      dealData.dealSummary.dealSignal === "Attractive"
                        ? "#2d5f3f"
                        : dealData.dealSummary.dealSignal === "Cautious"
                        ? "#c33"
                        : "#1a1a1a",
                ...
                >
                  <span style={{ fontSize: "0.875rem", marginTop: "0.125rem" }}>
                    🔍
                  </span>
                  <p
                    style={{
        ...
        >
          ...
          >
            ...
            <MetricCard
              icon="💰"
              label="Profit Margin"
              value={`${dealData.metrics.profitMargin}%`}
            />
            <MetricCard
              icon="📊"
              label="Debt Ratio"
              value={dealData.metrics.debtRatio}
            />
            <MetricCard
              icon="💵"
              label="Current Ratio"
              value={dealData.metrics.currentRatio}
              positive={parseFloat(dealData.metrics.currentRatio) > 1.5}
            />
            <MetricCard
              icon="📈"
              label="EV / EBITDA"
              value={`${dealData.metrics.evToEbitda}x`}
            />
            <MetricCard
              icon="⚡"
              label="Debt-to-EBITDA"
              value={`${dealData.metrics.debtToEbitda}x`}
            />
            <MetricCard
              icon="💸"
              label="Cash Flow"
              value={`$${dealData.metrics.cashFlow}M`}
            />
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    backgroundColor:
                      qaLoading || !qaQuestion.trim() ? "#e5ddd5" : "#d4a574",
                    border: "none",
                    borderRadius: "6px",
                    color: "white",
                    fontSize: "0.875rem",
